...
public:
    Indexer();
    void IndexQuestion(char* input_file);
    void IndexTags(char* input_file);
    void storeInAVLTree(string&, int&);
    void storeInHashTable(string&, int&);
    void clearIndex();
    int getAmountWords();
    int getAmountQuestions();
    vector<int> getIDNumsHash(string&);
    vector<int> getIDNumsAVL(string&);



    void passCommandArgs(char* argv[]);

    void checkID(int, char*);
    void printMenu(vector<vector<char*>>);
    ...
    void readIndexFromFile(char);

private:

    char ** arg;
    int idParseCount = 0;
    vector<int> suggestedIdNums;
    int questionsIndexed;


    void trunkateWord(string title, int&);
    bool isStopWord(string tokString);

    int tagNum;
    int questionNum;
    const char* saveFilePath;

    StemmingAlgorithm stemmer;
    AVLTree<WordObject> tree;
    HashTable<string, int> table;
    vector<string> stopWordList = {"a","a's","able","about","above","abroad","according","accordingly",
                                   ...
                                   "awfully","back","backward","backwards","be","became","because","become",
                                   "becomes","becoming","been","before","beforehand","begin","behind","being",
                                   "believe","below","beside","besides","best","better","between","beyond","both",
                                   "brief","but","by","c'mon","c's","came","can","can't","cannot","cant",
                                   "caption","cause","causes","certain","certainly","changes","clearly","co","co.",
                                   ...
                                   "currently","dare","daren't","definitely","described","despite","did","didn't",
                                   "different","directly","do","does","doesn't","doing","don't","done","down",
                                   ...
                                   "everywhere","ex","exactly","example","except","fairly","far","farther","few",
                                   ...
                                   ,"of","off","often","oh","ok","okay","old","on","once","one","one's","ones"
                                   ...
                                   "specify","specifying","still","sub","such","sup","sure","t's","take","taken",
                                   "taking","tell","tends","th","than","thank","thanks","thanx","that","that'll",
                                   "that's","that've","thats","the","their","theirs","them","themselves","then",
                                   "thence","there","there'd","there'll","there're","there's","there've",
                                   ...
                                   "tries","truly","try","trying","twice","two","un","under","underneath","undoing"
                                   ...
                                   "what","what'll","what's","what've","whatever","when","whence","whenever",
                                   "where","where's","whereafter","whereas","whereby","wherein","whereupon",
                                   "wherever","whether","which","whichever","while","whilst","whither","who",
                                   "who'd","who'll","who's","whoever","whole","whom","whomever","whose","why","will"
